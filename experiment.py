...
def test(args):
    paths, saved_config, saved_seed = _training_metadata(args.experiment)
    config_name = args.config or saved_config
    if config_name is None:
        raise SystemExit(
            "No saved training metadata exists. Pass --config explicitly, "
            "for example: "
            "python experiment.py test <experiment> --config <config_name>"
        )
    ...
    _append_override(command, "--seed", args.seed if args.seed is not None else saved_seed)
    _append_override(command, "--num_workers", args.num_workers)
    _append_override(command, "--batch_size", args.batch_size)
...
def build_parser():
    ...
    train_parser.add_argument("--config", required=True, help="registered config name")
    ...
    test_parser.add_argument("--config", help="override or supply a training config")
